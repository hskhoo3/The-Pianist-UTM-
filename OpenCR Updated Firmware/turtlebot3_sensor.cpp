...
{
  sonar_pin_front.trig = BDPIN_GPIO_1;
  sonar_pin_front.echo = BDPIN_GPIO_2;

  pinMode(sonar_pin_front.trig, OUTPUT);
  pinMode(sonar_pin_front.echo, INPUT);

  sonar_pin_left.trig = BDPIN_GPIO_3;
  sonar_pin_left.echo = BDPIN_GPIO_4;

  pinMode(sonar_pin_left.trig, OUTPUT);
  pinMode(sonar_pin_left.echo, INPUT);

  sonar_pin_right.trig = BDPIN_GPIO_5;
  sonar_pin_right.echo = BDPIN_GPIO_6;

  pinMode(sonar_pin_right.trig, OUTPUT);
  pinMode(sonar_pin_right.echo, INPUT);

  sonar_pin_back.trig = BDPIN_GPIO_7;
  sonar_pin_back.echo = BDPIN_GPIO_8;

  pinMode(sonar_pin_back.trig, OUTPUT);
  pinMode(sonar_pin_back.echo, INPUT);
}


void Turtlebot3Sensor::updateSonarFront(uint32_t t)
{
  ...
  if (t - t_time >= 10){
      digitalWrite(sonar_pin_front.trig, LOW);
      duration = pulseIn(sonar_pin_front.echo, HIGH);
      distance = ((float)(340 * duration) / 10000) / 2;
      t_time = t;
      digitalWrite(sonar_pin_front.trig, HIGH);
  }
  else
      digitalWrite(sonar_pin_front.trig, HIGH);	
  sonar_data_front = distance;
}

void Turtlebot3Sensor::updateSonarLeft(uint32_t t)
{
  static uint32_t t_time = 0;
  ...
  if (t - t_time >= 10){
      digitalWrite(sonar_pin_left.trig, LOW);
      duration = pulseIn(sonar_pin_left.echo, HIGH);
      distance = ((float)(340 * duration) / 10000) / 2;
      t_time = t;
      digitalWrite(sonar_pin_left.trig, HIGH);
  }
  else
      digitalWrite(sonar_pin_left.trig, HIGH);	
  sonar_data_left = distance;
}

void Turtlebot3Sensor::updateSonarRight(uint32_t t)
{
  static uint32_t t_time = 0;
  ...
  if (t - t_time >= 10){
      digitalWrite(sonar_pin_right.trig, LOW);
      duration = pulseIn(sonar_pin_right.echo, HIGH);
      distance = ((float)(340 * duration) / 10000) / 2;
      t_time = t;
      digitalWrite(sonar_pin_right.trig, HIGH);
  }
  else
      digitalWrite(sonar_pin_right.trig, HIGH);	
  sonar_data_right = distance;
}

void Turtlebot3Sensor::updateSonarBack(uint32_t t)
{
  static uint32_t t_time = 0;
  ...
  if (t - t_time >= 10){
      digitalWrite(sonar_pin_back.trig, LOW);
      duration = pulseIn(sonar_pin_back.echo, HIGH);
      distance = ((float)(340 * duration) / 10000) / 2;
      t_time = t;
      digitalWrite(sonar_pin_back.trig, HIGH);
  }
  else
      digitalWrite(sonar_pin_back.trig, HIGH);	
  sonar_data_back = distance;
}
...
float Turtlebot3Sensor::getSonarDataFront(void)
{
  float distance = 0.0;

  distance = sonar_data_front;

  return distance;
}

float Turtlebot3Sensor::getSonarDataLeft(void)
{
  float distance = 0.0;

  distance = sonar_data_left;

  return distance;
}

float Turtlebot3Sensor::getSonarDataRight(void)
{
  float distance = 0.0;

  distance = sonar_data_right;

  return distance;
}

float Turtlebot3Sensor::getSonarDataBack(void)
{
  float distance = 0.0;

  distance = sonar_data_back;
