...
class Turtlebot3Sensor
{
 public:
  ...
  // Sonar sensor
  void initSonar(void);
  void updateSonarFront(uint32_t t);
  void updateSonarLeft(uint32_t t);
  void updateSonarRight(uint32_t t);
  void updateSonarBack(uint32_t t);

  float getSonarDataFront(void);
  float getSonarDataLeft(void);
  float getSonarDataRight(void);
  float getSonarDataBack(void);
 ...
 private:
  ...
  SonarPin sonar_pin_front, sonar_pin_left, sonar_pin_right, sonar_pin_back;

  float sonar_data_front, sonar_data_left, sonar_data_right, sonar_data_back;
};
